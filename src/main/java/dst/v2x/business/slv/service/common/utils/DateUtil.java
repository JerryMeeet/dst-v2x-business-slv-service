...
public class DateUtil {
	...
	public static String DIFFTIMEFORMAT = "yyyy/MM/dd HH:mm:ss";
	...
	public static Date getYesterday(Date date) {
		Objects.requireNonNull(date, "the date is null or empty!");
...
//		Date incomeDate = getYesterday(new Date());
//		System.out.println(setStartDay(incomeDate).getTime());
//		System.out.println(setEndDay(incomeDate).getTime());
//		System.out.println(getCurrentDateYMDHMSStr());
//		System.out.println(StrUtil.getRandom(5));
//	}
	 ...
	 * @return
	 */
	public static String getDatePoor(Date endDate, Date beforeDate) {
		// 天
		...
		// 获得两个时间的毫秒时间差异
		long diff = endDate.getTime() - beforeDate.getTime();
		// 计算差多少天
		long day = diff / nd;
		// 计算差多少小时
	 ...
	 * @return
	 */
	public static String getDateHourPoor(Date endDate, Date beforeDate) {
		// 天
		long nd = 1000 * 24 * 60 * 60;
		// 时
		long nh = 1000 * 60 * 60;
...
//		long min = diff % nd % nh / nm;
	...
	public static boolean isBetween(Date startDate, Date endDate,Date compareDate) {
		...
		return compareDate.before(endDate) && compareDate.after(startDate);
	}
	
	/**
	 * Description: 当前时间距离下个工作日一三五早上9点30分的毫秒数 <br/>
	 * Created By: fengkaixuan <br/>
	 * Creation Time: 2021年2月25日 下午5:16:35 <br/>
	 * <br/>
	 * @return <br/>
	 */
	public static Long timeLag() {
		Calendar calendar = Calendar.getInstance();
		// 周是从周日为第一天开始计算的(默认的), 也可以自己设置每周的第一天
		int dayOfWeek = calendar.get(Calendar.DAY_OF_WEEK);
		...
		if (dayOfWeek < 2) {
			...
			if (dayOfWeek == 2 && hourOfDay <= 9 && minute <= 30) {
				day = 2;
			} else {
				day = 4;
			}
		} else if (dayOfWeek >= 4 && dayOfWeek < 6) {
			if (dayOfWeek == 4 && hourOfDay <= 9 && minute <= 30) {
				day = 4;
			} else {
				day = 6;
			}
		} else {
			if (dayOfWeek == 6 && hourOfDay <= 9 && minute <= 30) {
				day = 6;
			} else {
				crossDay = true;
				day = 2;
		...
		return diffTimes(calendar.getTime(), endDate.getTime());
	}
	...
	public static Date getCurrentDateStart() {
		...
		c.set(Calendar.DAY_OF_MONTH,1);//设置为1号,当前日期既
		return c.getTime();
	}
	...
	// 获取某日期前n个小时开始时间 00:00
	public static Date getBeforeHourStart(Date date, int hour) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		cal.set(Calendar.MINUTE, 00);
		cal.set(Calendar.SECOND, 00);
	...
	// 获取某日期前n个小时结束时间 00:00
	public static Date getBeforeHourEnd(Date date, int hour) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		cal.set(Calendar.MINUTE, 00);
		cal.set(Calendar.SECOND, 00);
